...
// - Repetir plan: ciclo de 28 días durante ~3 meses (sin tocar localStorage) hasta nuevo JSON.
...
// - Semana: mostrar prioridad y permitir gesto de deslizar para cambiar semana.
...
function App() {
  ...
  const onTouchEndDay = (e)=>{ if(!e.changedTouches?.length) return; const dx=e.changedTouches[0].clientX-tX.current; const dy=e.changedTouches[0].clientY-tY.current; if(Math.abs(dx)>40 && Math.abs(dx)>Math.abs(dy)*1.3){ const step = dx<0?1:-1; const nd=skipWeekends(addDays(viewDate, step)); setViewDate(nd); setSelectedDate(nd);} };
  const onTouchEndWeek = (e)=>{ if(!e.changedTouches?.length) return; const dx=e.changedTouches[0].clientX-tX.current; const dy=e.changedTouches[0].clientY-tY.current; if(Math.abs(dx)>40 && Math.abs(dx)>Math.abs(dy)*1.3){ const nd=addDays(viewDate, dx<0?7:-7); setViewDate(nd);} };
  ...
  const saveEdit = ()=>{
    ...
    if (editDraft.mode==="lista"){
      ...
      if (hasDuplicateForDay(key, editDraft.clienteId, clientsById[editDraft.clienteId]?.nombre||"", editingId)) { setErrorEdit("Ya existe una nota para ese cliente en ese día."); return; }
                    ...
                    .map(it=>
                      <li key={it.id+"@"+it.__origin} className="px-4 py-3 space-y-1 bg-[rgba(0,0,0,0.015)]">
                        <div className="flex items-start gap-3">
                          ...
                          {/* No edición aquí para simplificar */}
                        </div>
                      </li>
                    )}
                </ul>
        ...
        <div className="px-4 pb-4 flex justify-end gap-2">
          ...
          <button onClick={()=>{
            try{
              ...
              localStorage.setItem("agenda.version","6");
